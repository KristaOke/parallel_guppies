...
#data<- read.csv('../Data/data.csv', header=TRUE, sep=",") %>% 
    #rename( Mean = MeanValue, Treatment = Predation) #this is just my old path read in I will delete when we have common path working
...
```{r R2 plots box}
ggplot(data = new.data, aes(x=TraitType2, y = R.2, fill = TraitType2)) + geom_boxplot() 
ggplot(data = new.data, aes(x=TraitType1, y = R.2, fill = TraitType1)) + geom_boxplot() 

d<-new.data %>% 
  filter(`n_distinct(Slope)`==1)
ggplot(data = d, aes(x=Slope, y = R.2, fill = Slope)) + geom_boxplot() 

ggplot(data = new.data, aes(x=StudyType, y = R.2, fill = StudyType)) + geom_boxplot() 
```
