...
colnames(output.perm)<- rep(c("TraitID", "R2", "R2adj"),555) #rename the columns as data type
...
resampleR2<-data.frame((unlist(output.perm$TraitID)), (unlist(output.perm$R2)), (unlist(output.perm$R2adj))) %>% 
  mutate(R2 = as.numeric(2), TraitID = as.numeric(1), R2adj = as.numeric(3)) %>% 
  select(4:6)
return(resampleR2)
...
```{r creating resample df}
resampleR2<-anova_loop.resample(dat = new.data)
...
###########
rdat.w.exp <- left_join(output.all, R2means, by = 'TraitID') #merge by TraitID (same col name)
summary(rdat.w.exp)
...
output.all<-read.csv(paste(wd,'/Data/TraitR2_among.csv',sep=""))
output.south<-read.csv(paste(wd,'/Data/TraitR2_South.csv',sep="")) #traits_s
output.caroni<-read.csv(paste(wd,'/Data/TraitR2_Caroni.csv',sep=""))#traits_d
output.south.drainages<-read.csv(paste(wd,'/Data/TraitR2_Among_Drainage.csv',sep="")) #traits_d_all
output.intros<-read.csv(paste(wd,'/Data/TraitR2_intro.csv',sep="")) #traits_i
output.intros.broad<-read.csv(paste(wd,'/Data/TraitR2_intro_broad.csv',sep="")) #traits_i_broad
...
```{r function to make them all}
# load in all data tables as unique outputs
#The trait filter groups are in Tallies.rmd, commented the corresponding df
output.all<-read.csv(paste(wd,'/Data/TraitR2_among.csv',sep="")) #new.data (all traits)
output.south<-read.csv(paste(wd,'/Data/TraitR2_South.csv',sep="")) #traits_s
output.caroni<-read.csv(paste(wd,'/Data/TraitR2_Caroni.csv',sep=""))#traits_d
output.south.drainages<-read.csv(paste(wd,'/Data/TraitR2_Among_Drainage.csv',sep="")) #traits_d_all
output.intros<-read.csv(paste(wd,'/Data/TraitR2_intro.csv',sep="")) #traits_i
...
output.re<-as.data.frame(output.re)
colnames(output.re)<- rep(c("TraitID", "R2", "R2adj"),555) #rename the columns as data type

#rearranges the df to make a proper long df
output.re<-pivot_longer(output.re, cols = 1:3, names_to = 'type') 
output.re<-pivot_wider(output.re, names_from = type, values_from = value)
TraitID<-unlist(output.re$TraitID)
R2<-unlist(output.re$R2)
R2adj<-unlist(output.re$R2adj)
resampleR2<-data.frame(TraitID, R2, R2adj) %>% 
  mutate(R2 = as.numeric(R2), TraitID = as.numeric(TraitID)) %>% 
  arrange(TraitID)
